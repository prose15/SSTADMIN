...
export const Accept=async(id)=>{
  ...
  if(level=='L2'){
 await updateDoc(docRef,{L2status:'accept'}).then(()=>{
    ...
    await  updateDoc(docRef,{L3status:'accept'}).then(()=>{
          console.log('accepted');
          nav('/timesheet/requests')
      }).catch((err)=>{
          console.log(err);
    ...
    if(level=='L2'){
      await  updateDoc(docRef,{L4status:'accept',status:'success'}).then(()=>{
        console.log('accepted');
        nav('/timesheet/requests')
    }).catch((err)=>{
        console.log(err);
...
if(Cookies.get('team')=='Product'){
  if(level=='L2'){
    await updateDoc(docRef,{L2status:'accept',status:'success'}).then(()=>{
         console.log('accepted');
         nav('/timesheet/requests')
     }).catch((err)=>{
         console.log(err);
